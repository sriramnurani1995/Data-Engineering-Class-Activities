...
file_name = f"archive_{today}.json.gz"  # Updated file extension to indicate compression
...
def upload_data_to_gcs(data):
    """Append compressed data to the JSON file in GCS."""
    retry_count = 0
    max_retries = 5
    while retry_count < max_retries:
        try:
            existing_data = ""
            if blob.exists():
                existing_data = zlib.decompress(blob.download_as_string()).decode('utf-8')
